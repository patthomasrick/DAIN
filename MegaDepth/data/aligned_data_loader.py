...
class AlignedDataLoader(BaseDataLoader):
    def __init__(
        self, _root, _list_dir, _input_height, _input_width, _is_flip, _shuffle
    ):
        transform = None
        dataset = ImageFolder(
            root=_root,
        ...
        data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=16, shuffle=_shuffle, num_workers=int(3)
        )
...
class AlignedDataLoader_TEST(BaseDataLoader):
    def __init__(self, _root, _list_dir, _input_height, _input_width):
        ...
        dataset = ImageFolder_TEST(
            root=_root,
            list_dir=_list_dir,
            _input_height=_input_height,
            _input_width=_input_width,
        )

        data_loader = torch.utils.data.DataLoader(
            dataset, batch_size=1, shuffle=False, num_workers=int(3)
        )
        self.dataset = dataset
        flip = False
        self.paired_data = PairedData(data_loader, flip)
