...
class ImageFolder_TEST(data.Dataset):
    ...
    def load_SfM_ORD(self, img_path, targets_path):
        ...
        resized_sfm_img = resize(
            sfm_image, (self.input_height, self.input_width), order=1
        )
    ...
    def __getitem__(self, index):
        ...
        sparse_sift_path = (
            self.root + "/sparse_features/" + folder_name + "/" + img_name + ".h5"
        )
        ...
        if not os.path.isfile(sparse_sift_path) or not os.path.isfile(img_path):
            ...
            img, y_A, x_A, y_B, x_B, ordinal = self.load_SfM_ORD(
                img_path, sparse_sift_path
            )
