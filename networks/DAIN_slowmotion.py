...
class DAIN_slowmotion(torch.nn.Module):
    def __init__(self, channel=3, filter_size=4, timestep=0.5, training=True):
        ...
        i = 0
        (
            self.initScaleNets_filter,
            self.initScaleNets_filter1,
            self.initScaleNets_filter2,
        ) = self.get_MonoNet5(
    ...
    def forward(self, input):
        ...
        for temp_0, temp_1, timeoffset in zip(
            ...
            cur_offset_output = [
                temp_0,
                temp_1,
            ]  # [cur_offset_outputs[0][0], cur_offset_outputs[1][0]]
    ...
    def FilterInterpolate(ref0, ref2, offset, filter, filter_size2, time_offset):
        ...
        return (
            ref0_offset * (1.0 - time_offset) + ref2_offset * (time_offset),
            ref0_offset,
            ref2_offset,
        )
